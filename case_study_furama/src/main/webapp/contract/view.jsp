            ...
            <div style="padding: 40px 25px" class="table-wrapper">
                ...
                <table id="employeeTable" class="table table-striped table-hover">
                    ...
                    <c:forEach var="contractList" items="${contractList}">
                        <tr>
                            <td><c:out value="${contractList.contractId}"></c:out></td>
                            <td><c:out value="${contractList.contractStartDate}"></c:out></td>
                            <td><c:out value="${contractList.contractEndDate}"></c:out></td>
                            <td><c:out value="${contractList.contractDeposit}"></c:out></td>
                            <td><c:out value="${contractList.contractTotalMoney}"></c:out></td>
                            <td><c:out value="${contractList.employee.employeeName}"></c:out></td>
                            <td><c:out value="${contractList.customer.customerName}"></c:out></td>
                            <td><c:out value="${contractList.service.serviceName}"></c:out></td>
                            <td>
                                <a href="/employee?action=edit&id=${contractList.contractId}" class="btn btn-success btn-edit"><i class="btn fas fa-edit"></i></a>

                                <!-- Button trigger modal -->
                                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#deleteModal-${contractList.contractId}">
                        ...
                        <!-- Modal -->
                        <div class="modal fade" id="deleteModal-${contractList.contractId}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        Are you sure to remove the employee with ID: ${contractList.contractId} & Name: ${contractList.contractStartDate}?
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                                        <a href="/employee?action=delete&id=${contractList.contractId}" class="delete"><button type="button" class="btn btn-primary">Delete Now</button></a>
                                    </div>
                                </div>
                            </div>
                        </div>
